...
class Address
{
     ...
     * Author: JiaMeng <666@majiameng.com>
     */
    private function loadAreaArray()
    {
        return unserialize(file_get_contents(__DIR__.'/lib/area-library.txt'));
    }
     ...
     * @return array
     */
    private function parseAddressDetail($a1, $a2, $a3)
    {
        $r = array();
        if ($a3 != '') {
            $area3_matches = array();
            ...
            if ($area3_matches && count($area3_matches) > 1) {
                if ($a2) {
                    foreach ($this->area['city'] as $id => $v) {
                    ...
                    if ($area2_matches) {
                        foreach ($area3_matches as $id => $v) {
                            ...
                            if (isset($area2_matches[$v['pid']])) {
                                ...
                                $r['province'] = $this->area['province'][$sheng_id]['name'];
                            }
                        }
                    }
                } else {
                ...
                $city = $this->area['city'][$city_id];
                $province = $this->area['province'][$city['pid']];
